...
// (that means you can open index.html in CHROME and it just works)
// (it also means you don't need a server, so you can just edit the files, press save and then refresh the page to see changes)
...
var app = {
    ...
    init: function() {
        ...
        // there are a few patterns to deal with the 'later' part - we're going to use 'Promises' (which is what the .then() is all about)
    ...
    // 'DOM' is going to start as an empty object on our core app object
    ...
    // caching the DOM elements we need ahead of time makes our app faster, and clearer to read
    ...
    bindEvents: function() {
        ...
        this.DOM.themeDefault.addEventListener('click', () => {
            ...
            this.render(null, null, 'themeDefault')
           
    ////        mixpanel.track('Change Theme');
    //        mixpanel.register({
     //           'Theme Name': this.render.theme()
      //      })
    ...
    render: function(htmlObject, location, theme) {
        ...
        // otherwise if render() gets a 'theme', apply it
        else if (theme) {
...
// REGISTERING SUPER PROPERTIES COULD POTENTIALLY GO HERE:
//            mixpanel.track('Change Theme');
       //     mixpanel.register({
        //        'Theme Name': this.render.theme()
       //     })
    ...
    destroyData: function() {
        ...
        if (numRows > 1) {
            // we're going to decrement using a for loop; because we want to remove the LAST row FIRST
    ...
    checkUser: function() {
        ...
        if (storedUser) {
            ...
            mixpanel.identify(storedUser.uuid);
        } else {
    ...
    getUserId: function() {
        ...
        // ^ so the first thing the function has to do is return a promise (which is an object) 
    ...
    // we'll finally render that HTML to the DOM using render(), which is going to give our app the SPA feel
    ...
    templates: {
            ...
            return new Promise(function(resolve, reject) {
                ...
                app.getIconData(coin).then(function(imageBlob) {
                    ...
                    htmlObject.addEventListener('click', function() {
                        ...
                        once: true
                    }); // nifty little way to make sure a listener can only be called once: https://mzl.la/2vbjZBU
        ...
        buildTableRowHTML: function(coin) {
            ...
            var HTMLstring = `<td class="coinSign ${coin.shortCode}">${coin.symbol}</td>
                        ...
                        <td class="sentiment ${coin.shortCode}"><span class="good">😀</span><span class="meh">😶</span><span class="bad">😔</span></td>
            ...
            mixpanel.track('Crypto Added',{
                "Coin Value": coin.price,
                "Market Cap": coin.marketCap,
                "One Hour Change": coin.change1hr,
                "24 Hour Change":coin.change24hr,
        ...
        convertFiatToCryptoInputs: function(htmlObject, coin) {
            ...
            // so we don't repeat some much code that is really just doing the same thing twice
            ...
            mixpanel.track('Crypto Converted',{
                "Conversion Input Amount": Number(amountEntered.replace(/[^0-9.-]+/g, "")),
                "Coin Symbol": coin.symbol,
                "Conversion Value (USD)": Number(fiatInputBox.value.replace(/[^0-9.-]+/g, "")),
                "Conversion Method": "Crypto To USD",
                "Coin Value": coin.price,
                "Market Cap": coin.marketCap,
                "One Hour Change": coin.change1hr,
                "24 Hour Change":coin.change24hr,
            ...
            mixpanel.people.set({
                "Last Conversion": new Date().toISOString()
            })
            ...
            fiatInputBox.addEventListener('blur', function(event) {
                ...
                mixpanel.track('Crypto Converted', {
                    "Conversion Input Amount": Number(amountEntered.replace(/[^0-9.-]+/g, "")),
                    "Coin Symbol": coin.symbol,
                    "Conversion Value (crypto)": Number(cryptoInputBox.value.replace(/[^0-9.-]+/g, "")),
                    "Conversion Method": "USD to Crypto",
                    "Coin Value": coin.price,
                    "Market Cap": coin.marketCap,
                    "One Hour Change": coin.change1hr,
                    "24 Hour Change":coin.change24hr,
    ...
    // memebreak, just for fun :) ... doesn't fully adhere to our existing style
    memeBreak: function() {
        return new Promise(function(resolve, reject) {
            app.DOM.loader.style.display = "block";
            app.DOM.app.style.display = "none";
            const api_key = 'xh69N50txSiTKdvcLcuLXeATYMWetrUO';
            ...
            fetch(`https://cors-anywhere.herokuapp.com/${giphyAPI}${searchTerm}&api_key=${api_key}&limit=100`).then((response) => response.json()).then((json) => {
                ...
                app.getIconData(gifURL).then(function(imageBlob) {
                    ...
                    resolve(htmlObject)
                        ...
                        mixpanel.track('Meme Break');
                        mixpanel.people.set({
                "Last Meme Break": new Date().toISOString()
                })
